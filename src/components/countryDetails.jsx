...
export default function CountryDetails() {
  const { isLightMode, lightOrDark, apiData, loading } = useGlobalContext()
  ...
  const countryDetail = apiData?.find(country => country.name.common === countryName)
  const { flags,
    name,
    population,
    region,
  ...
  const getBorders = () => {
    if (borders !== undefined) {
      const borderCountries = borders?.map(bord => {
        const borderCountry = apiData?.find(item => (item.cioc === bord || item.fifa == bord || item.cca3 == bord))
  ...
  const borderElements = () => {
    if (getBorders() !== undefined) {
      return getBorders().map((thisCountry, index) => (
        <Link to={`/${thisCountry?.name.common}`} key={index}
